...
class BusinessAccountV3CheckSpec
    extends BaseSpec
    with MockServer
  ...
  "/verify/business" should {
    ...
    "return a cached Modulr response for subsequent calls" in {
      ...
      service.postVerifyBusiness(requestBody, xRequestId).status mustBe 200
    ...
    "not make a Module API call when the sort code is not on EISCD" in {
      ...
      val response = service.postVerifyBusiness(requestBody, xRequestId)
    ...
    "return a BAD_REQUEST (400) response" when {
      ...
      "should receive a forbidden request when calling the assess endpoint with an unknown user agent" taggedAs (LocalTests, ZapTests) in {
        ...
        val response    = service.postVerifyBusinessWithUnknownUserAgent(requestBody, xRequestId)
