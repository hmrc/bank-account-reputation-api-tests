...
class VerifyBusinessSpec extends BaseSpec with MockServer {
  ...
  "Invalid requests" when {
    ...
    "should receive a bad request when calling the assess endpoint with empty address lines" taggedAs (LocalTests, ZapTests) in {
      val requestBody = BusinessRequest(
        DEFAULT_ACCOUNT,
        Some(Business(companyName = generateRandomBusinessName, address = Some(Address(Some(Array()), Some("X9 9AG")))))
      )
      val response    = service.postVerifyBusiness(requestBody)
      val actual      = Json.parse(response.body).as[BadRequest]
    ...
    "should receive a bad request when calling the assess endpoint with missing address lines" taggedAs (LocalTests, ZapTests) in {
      val requestBody = BusinessRequest(
        DEFAULT_ACCOUNT,
        Some(Business(companyName = generateRandomBusinessName, address = Some(Address(postcode = Some("X9 9AG")))))
      )
      val response    = service.postVerifyBusiness(requestBody)
      val actual      = Json.parse(response.body).as[BadRequest]
    ...
    "should receive a bad request when calling the assess endpoint with an address line exceeding 140 chars" taggedAs (LocalTests, ZapTests) in {
      ...
      val requestBody      = BusinessRequest(
        ...
        Some(
          Business(
            companyName = generateRandomBusinessName,
            address = Some(Address(Some(Array(moreThan140chars)), Some("X9 9AG")))
          )
        )
      )
      val response         = service.postVerifyBusiness(requestBody)
      ...
      val requestBody = BusinessRequest(
        ...
        Some(
          Business(
            ...
            address = Some(
              Address(
                Some(Array("first line", "second line", "third line", "fourth line", "fifth line")),
                Some("X9 9AG")
              )
            )
          )
