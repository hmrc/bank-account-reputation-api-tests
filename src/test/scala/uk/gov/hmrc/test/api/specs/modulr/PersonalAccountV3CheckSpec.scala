...
class PersonalAccountV3CheckSpec
    extends BaseSpec
    with MockServer
  ...
  "/personal/v3/assess" should {
    ...
    "send correct headers to Modulr" in {
      ...
      service.postPersonalAssessV3(requestBody, xRequestId)
    ...
    "return a cached Modulr response for subsequent calls" in {
      ...
      service.postPersonalAssessV3(requestBody, xRequestId).status mustBe 200
    ...
    "return a OK (200) response with the expected assessment results" when {
      ...
      "the Modulr API responds with CLOSE_MATCH" in {
        ...
        // AssessV3 changes `partial` to `yes` for the nameMatches field, AccessV4 would leave it as `partial`
        responseBody.nameMatches mustBe "yes"
